...
/* ─── Divisor orgánico tipo nube ───────────────────────────
   Genera una fila de arcos suaves que conecta dos secciones
   de distinto color. `fill` = color de la sección siguiente. */
function ScallopDivider({ fill }: { fill: string }) {
  const width = 1440;
  const bump = 180;
  const base = 64;
  const height = 124;
  ...
  return (
    <svg
      ...
      className="block h-[72px] w-full sm:h-[104px]"
    >
      <path d={d} fill={fill} />
    </svg>
  );
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-void text-ink">
      ...
      <section className="relative isolate bg-accent text-void">
        <header className="absolute inset-x-0 top-0 z-20">
          ...
          >
            ...
            <div className="flex items-center gap-1 sm:gap-3">
              ...
              <Link
                href="/auth/signup"
                className="inline-flex min-h-11 items-center justify-center rounded-xl bg-void px-4 text-sm font-black text-ink transition-transform duration-200 hover:-translate-y-0.5 sm:px-5"
              >
                Crear cuenta
              </Link>
            </div>
        ...
        <div className="mx-auto flex w-full max-w-6xl flex-col items-center px-5 pb-10 pt-32 text-center sm:px-8 sm:pt-36">
          ...
          <h1 className="relative mt-10 max-w-full select-none text-[clamp(3rem,13vw,12rem)] font-black leading-[0.85] tracking-[-0.055em]">
            ...
            <Sticker className="absolute -left-2 bottom-6 hidden rotate-3 bg-ink text-void lg:inline-flex lg:-left-14">
              <WifiOff size={13} />
              Sin conexión
            </Sticker>
          </h1>
          ...
          <div className="mt-9 flex flex-col gap-3 sm:flex-row">
            ...
            <Link
              href="/auth/login"
              className="inline-flex min-h-13 items-center justify-center rounded-xl border-2 border-void/30 px-7 py-3.5 text-base font-bold text-void transition-colors hover:border-void/60 hover:bg-void/5"
            >
              Ya tengo cuenta
            </Link>
          </div>
      ...
      <section className="bg-accent text-void">
        ...
        <div className="mx-auto flex max-w-6xl flex-col items-center gap-10 px-5 pb-24 pt-10 text-center sm:px-8 sm:pb-28">
          ...
          <Link
            href="/auth/signup"
            className="group inline-flex min-h-13 items-center justify-center gap-3 rounded-xl bg-void px-8 py-4 text-base font-black text-ink shadow-[0_16px_40px_-12px_rgba(13,14,23,0.55)] transition-transform duration-200 ease-out hover:-translate-y-0.5"
          >
            Crear mi cuenta
            <ArrowRight
              size={18}
