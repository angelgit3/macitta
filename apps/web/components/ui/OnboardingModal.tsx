...
const SLIDES = [
    {
        ...
        bg: "bg-void/60 border border-ink-muted/15",
...
/**
 * OnboardingModal — step-by-step first-time user walkthrough.
 * Covers the three concept slides and a username-setup step.
 */
                    ...
                    {currentSlide && (
                        <div key={step} className="flex flex-col items-center text-center gap-5 animate-in fade-in slide-in-from-bottom-3 duration-300">
                            ...
                            <div>
                                ...
                                <p className="text-ink-faint text-sm leading-relaxed">{currentSlide.body}</p>
                            </div>
                            ...
                            <div className="flex gap-2">
                                {SLIDES.map((_, i) => (
                                    <div
                                        key={i}
                                        className={`h-1.5 rounded-full transition-all duration-300 ${i === step ? "w-6 bg-accent" : "w-1.5 bg-ink/20"}`}
                                    />
                                ))}
                            </div>
                        </div>
                    ...
                    {isUsernameStep && (
                        <div className="flex flex-col gap-5 animate-in fade-in slide-in-from-bottom-3 duration-300">
                            <div className="text-center">
                                ...
                                <p className="text-ink-faint text-sm">Elige cómo quieres ver tu nombre dentro de Macitta.</p>
                            </div>
                            ...
                            <div className="space-y-2">
                                ...
                                <input
                                    ...
                                    maxLength={32}
                                    autoFocus
                                    className="w-full soft-field rounded-2xl py-3.5 px-4 text-sm"
                                />
                            ...
                            >
                                {saving
                                    ...
                                    : <>Empezar a estudiar <ChevronRight size={16} /></>}
                            </button>

                            <button
                                onClick={skipToFinish}
                                className="text-xs text-ink-faint/60 hover:text-ink-faint transition-colors text-center"
                            >
                                Saltar por ahora
                            </button>
                        </div>
