...
import { BookOpen, Flame, Users, Zap, ChevronRight, CheckCircle2, Loader2 } from 'lucide-react';
...
const SLIDES = [
    ...
    },
    {
        ...
        body: 'Si tienes un maestro en Macitta, pídele su código de grupo. Desde tu dashboard podrás unirte con un solo clic.',
    },
] as const;
...
export function OnboardingModal({ userId, onDone }: OnboardingModalProps) {
    ...
    const [step, setStep] = useState(0); // 0..3 = slides, 4 = username setup
    const [username, setUsername] = useState('');
    const [saving, setSaving] = useState(false);
    const [error, setError] = useState<string | null>(null);

    const isLastSlide = step === SLIDES.length - 1;
    const isUsernameStep = step === SLIDES.length;
    ...
    async function skipToFinish() {
        // Mark done without username requirement
            ...
            <div className="w-full max-w-sm bg-stone-surface border border-border-subtle rounded-3xl overflow-hidden shadow-2xl">
                ...
                <div className="h-1 bg-white/5">
                    <div
                        ...
                        style={{ width: `${((step + 1) / (SLIDES.length + 1)) * 100}%` }}
                    />
                </div>
                ...
                <div className="p-7">
                    {!isUsernameStep ? (
                        ...
                        <div key={step} className="flex flex-col items-center text-center gap-5 animate-in fade-in slide-in-from-bottom-3 duration-300">
                            ...
                            <button
                                onClick={() => isLastSlide ? setStep(SLIDES.length) : setStep(s => s + 1)}
                            ...
                            >
                                {isLastSlide ? 'Configurar mi perfil' : 'Siguiente'}
